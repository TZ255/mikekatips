...
const sendTelegramNotification = async (message, disable_notification = false) => {
    try {
...
const sendTelegramPaymentConfirmed = async (message, disable_notification = false) => {
    try {
        const botToken = process.env.NOTIFY_BOT_TOKEN;
        if (!botToken) {
            console.log('Telegram bot token not configured.');
...
module.exports = { sendTelegramNotification, sendTelegramPaymentConfirmed };